...
export async function* extractPdfText(
    ...
    try {
        ...
        for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
            ...
            // Sort text items by position and reconstruct lines
            const lines = reconstructLines(textContent.items as TextItem[]);
 ...
 * Groups items by Y position to form lines
 */
function reconstructLines(items: TextItem[]): string[] {
    ...
    // Sort lines by Y position (descending, as PDF Y starts from bottom)
    const sortedYs = Array.from(lineMap.keys()).sort((a, b) => b - a);

    // Build lines by sorting items by X position within each line
    const lines: string[] = [];

    for (const y of sortedYs) {
        const lineItems = lineMap.get(y)!;
        // Sort by X position (transform[4])
        ...
        for (const item of lineItems) {
            ...
            if (line.length > 0) {
                const gap = x - lastX;
                if (gap > 20) {
                    line += '    '; // Large gap = multiple spaces
                } else if (gap > 5) {
                    line += ' '; // Small gap = single space
                }
            }
            line += item.str;
            lastX = x + item.width;
